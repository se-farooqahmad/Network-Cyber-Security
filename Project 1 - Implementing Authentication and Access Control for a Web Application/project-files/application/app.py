...
async def getAccessToken(request: Request, code: str | None = None):
    ...
    response.set_cookie(SESSION_COOKIE_NAME, access_token, httponly=True, max_age=SESSION_DURATION.total_seconds())
    return response
...
async def get_bill_adjustments(request: Request, user: dict = Depends(validate_session)):
    ...
    return templates.TemplateResponse("bill_adjustments.html", {"request": request})
