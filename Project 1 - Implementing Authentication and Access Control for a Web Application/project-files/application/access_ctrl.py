...
class AccessController:
   ...
   # def is_allowed(self, role: str, resource: str):
    ...
    def is_allowed(self, role: str, resource: str):
        """
        Checks if the given role has access to the requested resource.
        """
        print("is_allowed method called")
        if role not in self.roles:
            return False
        if resource not in self.resources:
            return False
        
        print("is_allowed running")
        role_index = self.roles.index(role)
        resource_index = self.resources.index(resource)
        has_permission = self.permissions[role_index][resource_index] == 1
        print(f"is_allowed has_permission: {has_permission}")

        return has_permission

    @staticmethod
    def get_role_from_username(username: str):
        """
        Maps username suffix to roles.
        """
        print(f"username: {username}")
        if username.endswith("_u1"):
            return "customer"
        elif username.endswith("_u2"):
            return "bank_cashier"
