def get_bill_data(connection, customer_id, connection_id, month, year):
    ...
    *
    ...
    bill_query = f"""
    select b.billissuedate, b.net_peakunits, b.net_offpeakunits, b.taxamount, b.fixedfee, b.arrears, b.duedate, b.totalamount_beforeduedate, b.totalamount_afterduedate, b.billid
    from bill b
    ...
    adjustments_query = f"""
    SELECT ADJ.AdjustmentAmount, ADJ.AdjustmentReason, ADJ.AdjustmentDate
    FROM BILLADJUSTMENTS ADJ
    WHERE ADJ.BILLID = '{bill_id}'
    """

    cursor.execute(adjustments_query)
    adjustments_data = cursor.fetchall()
    
    return customer_data, connections_data, bill_data, tariffs_data1, tariffs_data2, tariffs_data3, taxes_data, subsidy_data, ff_data, bills_prev_data, adjustments_data